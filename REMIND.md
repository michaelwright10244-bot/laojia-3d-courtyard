...
当前页面壳已切换为“梦幻 GIS 小城 / End-State Preview”展示口径：

- `CityViewport.vue` 新增英雄区指标、GIS 图层面板、仿真状态面板和梦幻导览文案。
- `src/styles.css` 新增极光背景、玻璃拟态面板、图层按钮和移动端适配。
- `src/scene/originalVisualScene.js` 新增 `gisOverlay`，并拆分 `terrain`、`poi`、`heat`、`story` 四个可开关图层。
- 道路 / 车流光轨图层已删除；人流 / 热力图层默认关闭，只在用户手动点击时显示。
- 左下 GIS 图层按钮已经能实际控制对应 Three.js 子图层显示；“节日 / 天气 / 故事”默认关闭，点击后显示灯会、湖岸烟花、归家故事和萤火粒子。
- 右上仿真状态会随生态暂停/恢复、故事层开关、热力层开关实时更新。
- 主屋比例继续微调：屋脊略压低，门窗尺度收紧，并新增勒脚、腰线、檐沟、入户三级台阶、窗台雨棚、电表箱等贴脸细节。

注意：这些都是叠加层和外壳表现，不应替换老家主体、原路网或已有功能区模型。
